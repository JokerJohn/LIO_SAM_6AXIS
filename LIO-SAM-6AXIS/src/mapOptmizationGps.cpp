    ...
    if (cloudKeyPoses3D->points.empty()) {
      ...
      if (useGPS) {
        ...
        if (syncGPS(gpsQueue, alignedGPS, timeLaserInfoCur,
          ...
          // Please note that the GPS point is the first point in the pose
          // graph, the first lidar pose should be  the second point, and the
          // GPS latitude and longitude data needs to be retained as the
          // origin of the localization map.
